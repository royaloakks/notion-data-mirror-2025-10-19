...
# Public ChatGPT-readable endpoint (not under /api/)
@app.get("/notion-content", response_class=HTMLResponse)
async def get_public_notion_content():
    """
    Public static HTML page with all synced content that ChatGPT can read
    This endpoint is at the root level (not /api/) for better accessibility
    """
    try:
        pages = await db.synced_pages.find({}, {"_id": 0}).to_list(1000)
        databases = await db.synced_databases.find({}, {"_id": 0}).to_list(1000)
...
""")
    ...
    <div class="content-section" id="page-{page['id']}">
        ...
        <div class="metadata">
            <span class="badge">page</span>
            <span>Last synced: {page.get('last_synced', 'Unknown')}</span>
            {f'<a href="{page["url"]}" target="_blank" rel="noopener">View in Notion →</a>' if page.get('url') else ''}
        </div>
        <div class="content-body">{content}</div>
    </div>
""")
    ...
    <div class="content-section" id="db-{database['id']}">
        ...
        <div class="metadata">
            <span class="badge database-badge">database</span>
            <span>Last synced: {database.get('last_synced', 'Unknown')}</span>
            {f'<a href="{database["url"]}" target="_blank" rel="noopener">View in Notion →</a>' if database.get('url') else ''}
        </div>
        <div class="content-body">{content}</div>
    </div>
""")
    ...
    <footer style="text-align: center; color: #9ca3af; font-size: 0.9em; padding: 20px 0;">
        <p>This page is generated from your Notion workspace and updates after each sync.</p>
        <p>ChatGPT and other AI assistants can read and answer questions about all the content on this page.</p>
    </footer>
</body>
</html>
""")
        ...
        logger.error(f"Error generating public notion content: {e}")
        return HTMLResponse(content=f"""
<!DOCTYPE html>
<html>
<head><title>Error</title></head>
